...
updateUserWordsCloudQueue.process(async (job) => {
    try{
        ...
        if(response && response['body'] && response['body']['data'] && Array.isArray(response['body']['data']) && response['body']['data'].length){
            await user.update({"_id": job['data']['userId']},{"$set":{"wordCloud": response['body']['data']}});
        }
    ...
    try{
        ...
        if(response && response['body'] && response['body']['data'] && Array.isArray(response['body']['data']) && response['body']['data'].length){
            ...
            job['data']['comments'].forEach(async(comment) => {
                ...
                count = count +1;
                await post.update({"twitterPostId": comment['twitterPostId']},{"$set":{"sentiment": s}})
...
class Workers {
    ...
    async fetchWordsCloud(userId, comments) {
        await updateUserWordsCloudQueue.add({userId: userId, comments: comments})
    }
    async saveSentiments(userId, comments) {
        await saveSentimentsQueue.add({userId: userId, comments: comments})
    }
}
